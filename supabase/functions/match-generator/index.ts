...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (updateError) {
      ...
      throw new Error(`Failed to mark job as processing: ${updateError.message}`)
    }
    ...
    console.log(`User profile: school=${typedProfile.school}, gender=${typedProfile.gender}`)
    ...
    console.log(`Filters: school="${typedProfile.school}", opposite_gender="${typedProfile.gender}", threshold=0.5, limit=20`)

    const { data: matches, error: searchError } = await supabase
      .rpc('find_similar_faces_filtered', {
    ...
    if (searchError) {
      ...
      if (typedJob.attempts + 1 < typedJob.max_attempts) {
        ...
        console.log(`Job marked for retry (attempt ${typedJob.attempts + 1}/${typedJob.max_attempts})`)
      } else {
    ...
    const matchRecords: MatchRecord[] = typedMatches.map((match) => ({
      face_a_id: typedJob.face_id < match.face_id ? typedJob.face_id : match.face_id,
      face_b_id: typedJob.face_id < match.face_id ? match.face_id : typedJob.face_id,
    ...
    console.log(`✅ Job ${typedJob.id} completed successfully with ${matchRecords.length} matches`)
