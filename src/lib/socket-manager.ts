...
import type { SocketEvents, SocketState, SocketStatus } from "@/types/socket";
...
export class SocketManager {
	...
	private setupEventHandlers(
		...
		Object.keys(this.socket._callbacks).forEach((event) => {
			if (
				!["connect", "disconnect", "connect_error", "error"].includes(event)
			) {
...
export const getSocketManager = () => {
	if (!socketManagerInstance) {
		throw new Error(
			"SocketManager not initialized. Call createSocketManager first.",
		);
	}
	return socketManagerInstance;
};
