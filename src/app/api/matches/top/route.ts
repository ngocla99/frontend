  ...
  try {
    ...
    const matchesWithUrls = await Promise.all(
      validMatches.map(async (match) => {
        ...
        const imageUrlA = urlA.data?.signedUrl ||
          supabase.storage.from("user-images").getPublicUrl(match.face_a.image_path).data.publicUrl;
        const imageUrlB = urlB.data?.signedUrl ||
          supabase.storage.from("user-images").getPublicUrl(match.face_b.image_path).data.publicUrl;
